"""
ZH -> VI Inference Script with Exact Architecture from train_rope_small.py
...
def main():
    ...
    output_dir = "/home/crl/hienhq/nlp/dataset/zh_vi_filtered_60"
    os.makedirs(output_dir, exist_ok=True)
    
    # Output files
    output_file = os.path.join(output_dir, "zh_vi_high_quality.txt")
    ...
    print(f"Starting ZH->VI translation with BLEU threshold {bleu_threshold}...")
    
    with open(log_file, 'w', encoding='utf-8') as log_f:
        for i, (zh_text, vi_ref) in enumerate(tqdm(zip(zh_sentences, vi_sentences), total=len(zh_sentences))):
            try:
                # Prepare input with direction token
                input_text = f"<2vi> {zh_text}"
                ...
                bleu_score = calculate_bleu(vi_ref, translated_text)
                
                # Log the translation
                log_f.write(f"Pair {i+1}:\n")
                log_f.write(f"ZH: {zh_text}\n")
                log_f.write(f"VI_REF: {vi_ref}\n") 
                log_f.write(f"VI_GEN: {translated_text}\n")
                log_f.write(f"BLEU: {bleu_score:.2f}\n\n")
                
                # Keep high-quality translations
                if bleu_score >= bleu_threshold:
                    high_quality_pairs.append((zh_text, translated_text))
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        for zh, vi in high_quality_pairs:
            f.write(f"{zh}\t{vi}\n")
