#!/usr/bin/env python3
"""Plan-CL contrastive finetuning starting from an existing RoPE checkpoint."""
...
class PlanCLFineTuneConfig(InferenceConfig):
    train_src_file: str = \
        "/home/crl/hienhq/olym/dataset/train_back_maxlen60/train_interleaved_maxlen60.src"
    train_tgt_file: str = \
        "/home/crl/hienhq/olym/dataset/train_back_maxlen60/train_interleaved_maxlen60.tgt"
    ...
    save_dir: str = "./checkpoints_plan_cl_finetune"
    save_every: int = 5
...
def load_interleaved_samples(src_path: str, tgt_path: str) -> List[Tuple[str, str]]:
    if not os.path.isfile(src_path) or not os.path.isfile(tgt_path):
        raise FileNotFoundError("Missing finetune training files.")
    ...
    if not samples:
        raise ValueError("Finetune dataset is empty after loading interleaved data.")
    return samples
...
def package_tokenizer(prefix: str) -> Dict[str, Any]:
    ...
    if not (os.path.isfile(model_path) and os.path.isfile(vocab_path)):
        raise FileNotFoundError(f"Tokenizer artifacts missing at {prefix}.[model|vocab]")
...
def split_samples(samples: Sequence[Tuple[str, str]], val_ratio: float) -> Tuple[List[Tuple[str, str]], List[Tuple[str, str]]]:
    total = len(samples)
    val_size = max(1, int(total * val_ratio))
    val_samples = list(samples[:val_size])
    train_samples = list(samples[val_size:])
    if not train_samples:
        raise ValueError("Training split is empty; reduce validation ratio.")
...
class PlanCLContrastiveDataset(Dataset):
    def __init__(
        self,
        samples: Sequence[Tuple[str, str]],
        sp_model: spm.SentencePieceProcessor,
        config: PlanCLFineTuneConfig,
        ...
        for zh_text, vi_text in samples:
            ...
            ids_zh_vi = self._encode_with_lang(config.vi_token, zh_text)
            ids_vi_vi = self._encode_with_lang(config.vi_token, vi_text)
            ids_vi_zh = self._encode_with_lang(config.zh_token, vi_text)
            ids_zh_zh = self._encode_with_lang(config.zh_token, zh_text)
        ...
        if not self.instances:
            raise ValueError("Contrastive dataset empty after filtering; adjust max/min lengths.")

    def _length_ok(self, tokens: Sequence[int]) -> bool:
        return self.config.min_len <= len(tokens) <= self.config.max_len

    def _encode_with_lang(self, lang_token: str, text: str) -> List[int]:
        seq = self.sp.encode(f"{lang_token} {text}", out_type=int)
        if not seq:
            return [self.pad_id]
        return seq[: self.config.max_len]
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "ids_zh_vi": torch.tensor(inst["ids_zh_vi"], dtype=torch.long),
            "ids_vi_vi": torch.tensor(inst["ids_vi_vi"], dtype=torch.long),
            "ids_vi_zh": torch.tensor(inst["ids_vi_zh"], dtype=torch.long),
            "ids_zh_zh": torch.tensor(inst["ids_zh_zh"], dtype=torch.long),
        }
...
def build_collate_fn(pad_id: int):
    ...
    def collate(batch: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
        return {
            ...
            "ids_zh_vi": _pad([item["ids_zh_vi"] for item in batch]),
            "ids_vi_vi": _pad([item["ids_vi_vi"] for item in batch]),
            "ids_vi_zh": _pad([item["ids_vi_zh"] for item in batch]),
            "ids_zh_zh": _pad([item["ids_zh_zh"] for item in batch]),
        }
...
def compute_crosslingual_loss(
    ...
    ids_zh_vi: torch.Tensor,
    ids_vi_vi: torch.Tensor,
    ids_vi_zh: torch.Tensor,
    ids_zh_zh: torch.Tensor,
    config: PlanCLFineTuneConfig,
    pad_id: int,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plan-CL contrastive finetuning from checkpoint")
    parser.add_argument(
        "--checkpoint",
        default="/home/crl/hienhq/olym/contrastive_v1/plan_cl_finetune_epoch_30.pt",
        help="Checkpoint (.pt) to load for initialization",
    )
    parser.add_argument("--train-src", default=None, help="Override training source file")
    parser.add_argument("--train-tgt", default=None, help="Override training target file")
    parser.add_argument("--val-ratio", type=float, default=0.001, help="Validation split ratio")
    parser.add_argument("--save-dir", default='contrastive_v2', help="Checkpoint directory for finetuned models")
    parser.add_argument("--save-every", type=int, default=10, help="Save checkpoint every N epochs")
    parser.add_argument("--num-epochs", type=int, default=50, help="Number of finetune epochs")
    parser.add_argument("--tokenizer-prefix", default=None, help="Fallback tokenizer prefix if checkpoint lacks payload")
    parser.add_argument("--batch-size", type=int, default=64, help="Override training batch size")
    parser.add_argument("--eval-batch-size", type=int, default=64, help="Override validation batch size")
    parser.add_argument("--lr-base", type=float, default=5e-5, help="Override AdamW base learning rate")
    parser.add_argument("--warmup-steps", type=int, default=2000, help="Override LR scheduler warmup steps")
...
def main() -> None:
    args = parse_args()
    default_config = PlanCLFineTuneConfig()
    config = PlanCLFineTuneConfig()
    ...
    config.batch_size = args.batch_size if args.batch_size is not None else default_config.batch_size
    config.eval_batch_size = args.eval_batch_size if args.eval_batch_size is not None else default_config.eval_batch_size
    config.lr_base = args.lr_base if args.lr_base is not None else default_config.lr_base
    config.warmup_steps = args.warmup_steps if args.warmup_steps is not None else default_config.warmup_steps

    os.makedirs(config.save_dir, exist_ok=True)
    set_seed(config.seed)

    tokenizer_tmp_dir = None
    if "tokenizer" in checkpoint:
        config.spm_prefix, tokenizer_tmp_dir = materialize_tokenizer(checkpoint["tokenizer"])
        if tokenizer_tmp_dir:
            atexit.register(lambda: shutil.rmtree(tokenizer_tmp_dir, ignore_errors=True))
    elif args.tokenizer_prefix:
        config.spm_prefix = args.tokenizer_prefix

    if not os.path.isfile(f"{config.spm_prefix}.model"):
        raise FileNotFoundError("SentencePiece model unavailable. Provide --tokenizer-prefix or ensure checkpoint packaged tokenizer.")

    sp_model = spm.SentencePieceProcessor()
    sp_model.Load(f"{config.spm_prefix}.model")
    vocab_size = sp_model.GetPieceSize()
    tokenizer_payload = checkpoint.get("tokenizer") or package_tokenizer(config.spm_prefix)
    ...
    train_dataset = PlanCLContrastiveDataset(train_samples, sp_model, config, include_dirs=("zh2vi", "vi2zh"))
    val_dataset = PlanCLContrastiveDataset(val_samples, sp_model, config, include_dirs=("zh2vi",))
    ...
    for epoch in range(1, config.num_epochs + 1):
        ...
        print(
            ...
            f"lambda: {metrics['val_lambda']:.4f} | Total: {metrics['val_total']:.4f}"
        )

        if epoch % config.save_every == 0:
            path = os.path.join(config.save_dir, f"plan_cl_finetune_epoch_{epoch}.pt")
            torch.save(
                {
                    "epoch": epoch,
        ...
        if metrics["val_total"] < best_val_total:
            ...
            best_path = os.path.join(config.save_dir, "plan_cl_finetune_best.pt")
            torch.save(
                {
                    "epoch": epoch,
                    "model_state_dict": model.state_dict(),
